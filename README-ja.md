...
Pair Secretの具体的な導出方法は本仕様の今後の版で定義する。
ただし、実装はPair SecretをPermanent-ID、Display Name、またはその他の公開情報のみから導出してはならない（MUST NOT）。
...
Peer Registry Entryは、後続のSession Establishmentにおいて、接続要求の照合、暗号方式の確認、およびセッション鍵導出に使用される。
